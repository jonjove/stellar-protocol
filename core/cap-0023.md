...
#### What is a PreparedTrustLineEntry?
A `PreparedTrustLineEntry` is a type of `LedgerEntry` with the following
properties

- It is not a sub-entry of any account
- It can be converted into a `TrustLineEntry` by `ChangeTrustOp`, and doing so
  also transfers the `reserve`
- It records the authorization state in `flags`, and it can have any
  authorization state that  a `TrustLineEntry` could have
- It records which account has provided the reserve in `createdBy`
- It records the earliest time at which it can be removed in `expiry`
- It can be removed if the current time is after `expiry`, but only by
  `createdBy`
- `createdBy` can change if another account is willing to either
    - pay the reserve for longer (eg. increase `expiry`)
    - pay a different reserve for the same amount of time (eg. same `expiry` but
      the `baseReserve` has changed)

This is just an overview of the properties. For specifics, please refer to the
detailed behavior of `CreatePreparedTrustLineOp`, `RemovePreparedTrustLineOp`,
`ChangeTrust`, and `AllowTrust`.
...
The behavior of `RemovePreparedTrustLineOp` is as follows:
...
accurate description of the behavior would be "ensure that either a trust line
exists or a prepared trust line exists that will not expire before the specified
time". In other words, `CreatePreparedTrustLineOp` does not fail if a trust line
or prepared trust line already exists. Instead the behavior is

- Succeed if a trust line exists
- Succeed if a prepared trust line exists and either
    - has a later expiry than the one you tried to create
    - has the same expiry and the same reserve as the one you tried to create
    - has an earlier expiry or the same expiry as the one you tried to create
      and you have `baseReserve` available balance of native asset
- Fail otherwise
