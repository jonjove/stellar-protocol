...
// PreauthorizeFlags is a bitwise combination of TrustlineFlags.
// Currently only AUTHORIZED_FLAG is permitted.
typedef uint32 PreauthorizeFlags;

union Claimant switch (ClaimantType type)
{
case CLAIMANT_TYPE_V0:
    ClaimCondition condition;       // Claimable if-and-only-if condition is satisfied
    PreauthorizeFlags preauthorize; // Preauthorization flags for destination
};
...
struct PreauthorizeClaimableBalanceOp
{
    ...
    PreauthorizeFlags preauthorize;
};
...
11. If `claimants[i].preauthorize & AUTHORIZED_FLAG` then set
    ...
    (in other words, set the `AUTHORIZED_FLAG` and clear the
    `AUTHORIZED_TO_MAINTAIN_LIABILITIES_FLAG` on the trust line)
...
### PreauthorizeClaimableBalanceOp does not allow AUTHORIZED_TO_MAINTAIN_LIABILITIES_FLAG
According to CAP-0018, the balance of a trust line with the
`AUTHORIZED_TO_MAINTAIN_LIABILITIES_FLAG` set can only change if an existing
liability is converted into a balance. Consider a variation of this proposal
in which `AUTHORIZED_TO_MAINTAIN_LIABILITIES_FLAG` is valid in
`PreauthorizeClaimableBalanceOp`. If a `ClaimableBalanceEntry` with that flag
set were claimed, then in step 11 of the behavior of `ClaimClaimableBalanceOp`
the `AUTHORIZED_TO_MAINTAIN_LIABILITIES_FLAG` on the trust line will be set.
But then the operation will still need to fail in step 12 because the trust
line will have `AUTHORIZED_TO_MAINTAIN_LIABILITIES_FLAG` instead of
`AUTHORIZED_FLAG`, so it will not be permitted to increase the balance in step
14.

Theoretically, it would be possible to relax the above invariant for
`ClaimClaimableBalanceEntry`. But what would be the benefit of doing so? An
issuer that is using the `AUTHORIZED_TO_MAINTAIN_LIABILITIES_FLAG` is indicating
that they intend to interact off-chain regularly with parties that hold their
asset. In this scenario, there is little reason to use
`AUTHORIZED_TO_MAINTAIN_LIABILITIES_FLAG` for pre-authorization; the claimant
would not be able to use the received funds without interacting with the issuer
anyway. From this analysis, it is clear that violating the above invariant
would increase complexity in the protocol without adding any useful
functionality.

### ClaimClaimableBalanceOp pre-authorization allows upgrade from AUTHORIZED_TO_MAINTAIN_LIABILITIES_FLAG to AUTHORIZED_FLAG
Consider a variation of this proposal in which pre-authorization is not
permitted to upgrade from `AUTHORIZED_TO_MAINTAIN_LIABILITIES_FLAG` to
`AUTHORIZED_FLAG`. If a `ClaimableBalanceEntry` with `AUTHORIZED_FLAG` set were
claimed by an account whose trust line had the
`AUTHORIZED_TO_MAINTAIN_LIABILITIES_FLAG` set, then the operation must fail
because the upgrade is not permitted. This has some important and confusing
consequences:

- `AUTHORIZED_TO_MAINTAIN_LIABILITIES_FLAG` as described in CAP-0018 is clearly
more permissive than no flags set, but it is less permissive here.
- If the trust line with `AUTHORIZED_TO_MAINTAIN_LIABILITIES_FLAG` had a balance
of 0, then the claimant account could remove the trust line before claiming the
`ClaimableBalanceEntry` thereby transitioning from
`AUTHORIZED_TO_MAINTAIN_LIABILITIES_FLAG` to `AUTHORIZED_FLAG` anyway.

Although I have no clear understanding of how this functionality would
_actually_ be useful, I recognize that forbidding this behavior will increase
complexity in the protocol while simultaneously removing functionality. An
issuer who sees no reason to use this functionality can choose not to do so.
